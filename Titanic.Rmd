...
La variable  **PassangerID** no se va a analizar ya que sirve para identificar a cada pasajero y presenta un valor diferente para cada observación (i.e. 891).  por lo que no puede ser indicativa de ninguna tendencia en los datos referente a la supervivencia.
...
### 4.1. Variable Pclass
Esta variable es indicativa de la clase a la que pertenece cada pasajero, y tiene valores entre 1 y 3, siendo 1 la clase más alta y la 3 la más baja. Como se ha visto con anterioridad no presenta ni valores NA ni vacíos.  Se analiza la distribución de la variable, no haciendo análisis de valores extremos debido a que se va a considerar una variable categórica. A continuación se observa su relación con la variable de supervivencia. 
...
En el diagrama de barras de las frecuencias relativas se observa que la mayor cantidad de pasajeros viajaban en tercera clase.
...
ggplot(
  ...
  ) + geom_bar(position="fill") + ylab("Frecuencia") + xlab("PClass")
```
...
# Gráfica
ggplot(data = datos,aes(x=Sex,fill=Survived))+geom_bar(position="fill")+ylab("Frecuencia")+xlab("Sex")
...
En primer lugar, analizamos la distribución de la variable Age.
...
ggplot(data = data_age, aes(x=Age_specified,  fill=Survived_age_specified))+geom_histogram(bins = 80)+ylab("Número")+xlab("Age")

# Gráfico para observar valores extremos

boxplot(data_age$Age_specified, main = "Age", ylab = "Años", col="orange")
...
# Gráfica
ggplot(data = datos,aes(x=Age_cat,fill=Survived))+geom_bar(position="fill")+ylab("Frecuencia")+xlab("Age")
...
ggplot(data = datos,aes(x=as.factor(SibSp),fill=Survived))+geom_bar(position="fill")+ylab("Frecuencia")+xlab("SibSp")
```
...
```{r}
g1 <- ggplot(data = datos,aes(x=as.factor(SibSp),fill=Sex))+geom_bar(position="fill")+ylab("Frecuencia")+xlab("SibsSp")
g2 <- ggplot(data = datos,aes(x=as.factor(SibSp),fill=Pclass))+geom_bar(position="fill")+ylab("Frecuencia")+xlab("SibsSp")
grid.arrange(g1, g2, ncol=1, nrow=2)
```

Tras analizar los gráficos anteriores, vemos que la mayoría de los pasajeros que viajaban solos eran hombres y esto explica porque la tasa de supervivencia es inferior pese a ser el grupo con mayor proporción de pasajeros en primera y segunda clase. Otro dato curioso es que las familias de más miembros tienen la tasa de supervivencia más baja, a su vez estos grupos pertenecen a tercera clase y en su mayoría eran hombres.

Para entender más los datos y la relación entre sus variables realizamos algunos test de chi-cuadrado entre la variable **SibSp** y **Sex**, **SibSp** y **PClass** y **SibSp** y Survived para entender si estas son dependientes o no.
...
Tras analizar estos datos, es posible sacar algunas conclusiones que explican la naturaleza de los datos. Por ejemplo, cuando dos o más personas tienen el mismo ticket y precio es que pertenecen  a la misma familia.
...
```{r}
ggplot(data = datos,aes(x=Parch,fill=Survived))+geom_bar(position="fill")+ylab("Frecuencia")
```
...
g1 <- ggplot(datos, aes(x=factor(Parch)))+
  geom_bar(stat="count", width=0.7, fill="steelblue")+xlab("Parch")+ylab("Número")

g2 <- ggplot(datos, aes(x=factor(SibSp)))+
  geom_bar(stat="count", width=0.7, fill="steelblue")+xlab("SibSp")+ylab("Número")
