...
```{r}
datos[datos$surnames=='Andersson' | datos$surnames=='Carter', ]
```
...
z_num <- mean(cabin_specified_set$Fare) - mean(cabin_not_specified_set$Fare)
z_den <- sqrt(
  ((sd(cabin_specified_set$Fare)^2)/length(cabin_specified_set$Fare))
  +
  ((sd(cabin_not_specified_set$Fare)^2)/length(cabin_not_specified_set$Fare))
)
z <- z_num / z_den
z
```
...
g1 <- ggplot(cabin_specified_set, aes(x=Fare)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666")+
 ggtitle('Cabin specified - Fare')

g2 <- ggplot(cabin_not_specified_set, aes(x=Fare)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666")+
...
```{r}
g1 <- ggplot(cabin_not_specified_set, aes(y = Fare))+geom_boxplot()
g2 <- ggplot(cabin_not_specified_set, aes(y = Fare))+geom_boxplot()+ylim(c(0,100))
grid.arrange(g1, g2, ncol=2, nrow=1)
```

```{r}
cuantiles <- quantile(cabin_not_specified_set$Fare)
threshold_cabin <- cuantiles[3] + (1.5 * (cuantiles[3]-cuantiles[1]))
```
...
datos$Cabin_cat <- as.factor(mapply(
  function (cabin, fare) if (cabin!="" && fare > threshold_cabin) "with_cabin" else "without_cabin",
  cabin=datos$Cabin,
  fare=datos$Fare
))
...
### 4.9. Resumen de los datos 
Se adjunta un resumen de los datos. 
```{r resumen fichero}
summary(datos)
```

### 4.10. Guardado de datos
