...
Como se puede observar, existen dos variables que tienen un valor único para cada observación: **PassengerId** y **Name**. Estas variables no se tendrán en cuenta en el análisis ya que no dan información diferencial para calcular la probabilidad de que un pasejero sobreviva o no. Además, **ticket**, es una variable categórica con 681 niveles, lo cual hace dificil su integración en los análisis que se van a efectuar más abajo. Por el momento se mantienen estas variables ya que pueden ser de ayuda para algún tratamiento de datos, pero para no sobreespecializar el algoritmo a nivel de la identidad de los pasajeros (nos interesa entender el comportamiento de las variables más genéricas) no se tendrán en cuenta para el análisis y se procederá con su eliminación antes de comenzar los análisis.
...
```
En el diagrama de caja mostrado anteriormente, es posible ver un valor máximo extremo que debido a ser el mayor vamos a analizarlo de forma individual.

```{r}
subset(datos[datos$Fare == max(datos$Fare), ], select = c(PassengerId, Ticket, Cabin, Embarked, Fare))
```

Vemos que este valor coincide en 3 registros cuyo ticket es el mismo. Por lo tanto, estas personas viajaban juntas y estaban en una de las cabinas más caras. Además, podemos ver un valor incorrecto que es que el pasajero 259 viajaba sin cabina cuando no tiene sentido viendo los demás tickets. Por lo tanto, aprovechamos para corregir este error.

```{r}
datos[datos$PassengerId == 259, ]$Cabin = "with_cabin"
```

A continuación, buscamos si existen otros registros cuyos tickets sean iguales y el valor de Cabina es diferente.

```{r TODO}
tickets_check<-c()
for (i in levels(datos$Ticket)){
 a <- length(unique(datos$Cabin[which(datos$Ticket==i)]))
 if(a>1) {
...
Usamos el valor más común de la cabina para estos registros ya que es se considera que es el más correcto. Aquellos registros cuyo ticket sea igual con diferente cabina y tengan dos no se modifican para evitar introducir más errores en los datos.

```{r}
for (ticket in tickets_check) {
  if (nrow(datos[datos$Ticket == ticket,]) > 2) {
    most_common <- names(which.max(table(datos[datos$Ticket == ticket, ]$Cabin)))
    datos[datos$Ticket==ticket, ]$Cabin = most_common
  }
}
```
